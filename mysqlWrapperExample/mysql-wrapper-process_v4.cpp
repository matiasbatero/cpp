...
class MySQL
{
	...
	void execute()
	{
		...
		long unsigned int* lengths[nfields];
		void* buffers[nfields];
		...
		{
			MYSQL_FIELD* aField = &res->fields[i];
			...
			lengths[i] = new long unsigned int(0);
			buffers[i] = malloc(aField->max_length);
			
			bind_out[0].buffer_type= MYSQL_TYPE_STRING;
			bind_out[0].is_null= 0;
			bind_out[0].buffer= (char *)buffers[i];
			bind_out[0].buffer_length= aField->max_length;
			bind_out[0].length= lengths[i];
		}
